...
static const uint32_t	g_sha224_constants[64] = {
...
uint32_t	 *sha224_hash(void *clear, uint64_t len)
{
	...
	if (clear == NULL)
		return (NULL);
	sha224_init(states);
	sha224_padding(clear, last_blocks, len);
	sha224_main_loop(states, clear, (uint32_t*)last_blocks, len);
	if (!(digest = malloc(sizeof(states) - sizeof(uint32_t))))
		return (NULL);
	if (ft_get_endianness())
	{
		i = 0;
	...
	ft_memcpy(digest, states, sizeof(states) - sizeof(uint32_t));
	return (digest);
}
