...
/*   Updated: 2018/07/18 08:56:07 by sclolus          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
INLINE static void	md5_padding(void *block_to_pad, char *clear, uint64_t total_len)
{
	...
	*((uint8_t*)block_to_pad + total_len % 64) |= 0x80;
	...
	((uint64_t*)block_to_pad)[7] = total_len * 8;
						  ...
						  , uint64_t len)
{
	...
	block_states[C] = states[C];
	block_states[D] = states[D];
	md5_round(block_states, (uint32_t*)(void*)last_block);
	states[A] += block_states[A];
	states[B] += block_states[B];
	states[C] += block_states[C];
	states[D] += block_states[D];
...
static char *md5_hash(char *clear)
{
	uint64_t		len;
	static uint8_t	last_block[64];
	uint32_t		states[4];
	char			*digest;

	if (clear == NULL)
		return (NULL);
	len = strlen(clear); // please fix strlen, you know it
//	assert(len == ft_strlen(clear));
...
int main(int argc, char **argv)
{
	if (argc == 2)
	{
		uint32_t *digest = (uint32_t*)(void*)md5_hash(argv[1]);
		if (digest == NULL)
			return (EXIT_FAILURE);
		printf("%8.8x%8.8x%8.8x%8.8x\n", swap_int32(digest[0]), swap_int32(digest[1]), swap_int32(digest[2]), swap_int32(digest[3]));
	}
	else
	{
